...
def test_release_admin_invitation_is_single_use_and_valid_for_24_hours(admin, organization):
...
def test_release_admin_invitation_rotates_an_existing_pending_invitation(admin, organization):
...
def test_release_admin_invitation_requires_active_admin_inviter(viewer, organization):
...
def test_release_admin_invitation_acceptance_is_single_use_and_sets_password(admin, organization):
    issue(admin, organization)
    invitation = Invitation.objects.get(email="release-admin@example.test")
    outbox = EmailOutbox.objects.get(idempotency_key=f"release-admin-invite:{invitation.id}")
