...
| Staging | Droplet Nginx serves the immutable SPA at `staging.vesselcalls.com` | Droplet `127.0.0.1:8010`, same-origin `/api` proxy | Separate managed DB, authenticated Redis on 6381, private bucket, Resend allow-list, Sentry project | Synthetic/anonymized only; never production credentials/data | **Prepared:** signed release deployment is gated on DNS/TLS, review, signing, and protected Droplet credentials; provider qualification remains deferred |
...
| Droplet staging slot | Release automation owner | Restricted `vessel-deploy` account; isolated staging service and release slot |

## Required DNS

- `vesselcalls.com`: production Droplet.
- `staging.vesselcalls.com`: Droplet staging Nginx, immutable SPA, and same-origin API.
- `staging-api.vesselcalls.com`: retained non-routable certificate endpoint; it does not expose the application.

As of 2026-07-26, Namecheap has:

- A `staging-api` → `146.190.76.23` (TTL one minute)

Before enabling the staging runtime, replace the legacy `staging` CNAME with
an A record to `146.190.76.23`, issue a Let’s Encrypt certificate for
`staging.vesselcalls.com`, and set `vessel_staging_frontend_tls_ready=true`.
...
Vercel is not part of the staging or production runtime. The legacy project is
left untouched; it receives no release artifact, token, proxy secret, or DNS
traffic. Spaces, Resend, and Sentry qualification remain `Deferred`—not passed,
waived, or complete.
