// LandingFull.tsx — the full dark-navy Vessel Caller marketing homepage,
...
type SvgProps = SVGProps<SVGSVGElement> & { size?: number; sw?: number; vb?: string };

function Svg({ size = 20, sw = 2, vb = "0 0 24 24", children, ...rest }: SvgProps) {
  return (
    <svg width={size} height={size} viewBox={vb} strokeWidth={sw} {...STROKE} {...rest}>
      {children}
    </svg>
  );
}
const IcAnchor = (p: SvgProps) => (
  <Svg {...p}>
    <path d="M12 22V8" />
...
const IcArrow = (p: SvgProps) => (
  <Svg sw={2.6} {...p}>
    <line x1="5" y1="12" x2="19" y2="12" />
    <polyline points="12 5 19 12 12 19" />
  </Svg>
);
const IcCheck = (p: SvgProps) => (
  <Svg sw={3} {...p}>
...
const PORTS = [
  "Port of Calabar", "Apapa Port, Lagos", "Tin Can Island Port", "Onne Port, Rivers",
...
const SMALL_FEATURES = [
  ...
  },
  {
    icon: (
      <Svg size={22} sw={1.8}>
        <path d="M12 2.7l5.66 5.66a8 8 0 1 1-11.32 0z" />
      </Svg>
    ),
    title: "Liquid & dry surveys",
...
const STEPS = [
  {
    ...
    body: "Company profile, operating ports, your logo, and a team with the right roles.",
  },
  {
    k: "02",
    icon: <IcAnchor sw={2} />,
    title: "Register the vessel call",
  ...
  },
  {
    ...
    body: "Console or quayside. Dues and commission preview before the surveyor submits.",
  },
  {
    k: "04",
    icon: (
    ...
    body: "The invoice issues itself at the locked rate. Record payment; receivables update everywhere.",
  },
];
...
// Footer columns: [heading, [href, label][]]. Hash hrefs stay in-page anchors;
...
const CONTAINERS: [number, number, number, string][] = [
    ...
    <>
      ...
      <header className="nav">
        ...
        <div
          ...
          onClick={(e) => { if ((e.target as HTMLElement).tagName === "A") closeMenu(); }}
        >
          {NAV.map(([href, label]) => <a key={href} href={href}>{label}</a>)}
          <Link to="/login" style={{ color: "var(--amber)", fontWeight: 700 }}>Launch app →</Link>
        </div>
      </header>
